...
impl RustletResponse {
	...
	pub fn write(&mut self, data: &[u8]) -> Result<(), Error> {
		...
		//info!("write data = {:?},data.len={}", data, data.len());
...
pub type Rustlet =
	fn(request: &mut RustletRequest, response: &mut RustletResponse) -> Result<(), Error>;
...
impl RustletContainer {
	pub fn new(config: RustletConfig) -> Self {
		let http = HttpServer::new(config.http_config.clone());

		RustletContainer { config, http }
	}

	pub fn start(&mut self) -> Result<(), Error> {
		self.http.config.callback = api_callback;
		self.http.start()?;
		self.http.add_api_extension("rsp".to_string())?;
		Ok(())
	}
	...
	pub fn add_rustlet_mapping(&mut self, name: &str, path: &str) -> Result<(), Error> {
		let mut rustlets = RUSTLETS.write().map_err(|e| {
			let error: Error = ErrorKind::InternalError(format!(
				"unexpected error: couldn't obtain RUSTLETS lock: {}",
				e.to_string(),
		...
		self.http.add_api_mapping(path.to_string())?;
